...
void update_distances_and_clusters_new(uint32_t gid,
  ...
  for (uint32_t i = 0; i < nclusters; i++) {
	distances[i] = 1.0-cos(PI*(1.0 - streamhash_similarity(graph_sketches[gid],centroid_sketches[i])));
	all_scores[gid].push_back(distances[i]);
    if (distances[i] < min_distance) {
      min_distance = distances[i];
      nearest_cluster = i;
    }
  ...
  // if distance > threshold: outlier
  if (min_distance > cluster_thresholds[nearest_cluster]) 
  {
  	// set cluster mapping to ANOMALY
  	cluster_map[gid] = ANOMALY;
	// set its anomaly score to distance from nearest centroid
  	anomaly_scores[gid] = min_distance;
  }
      ...
      int nearest_cluster_size = cluster_sizes[nearest_cluster];
      auto& centroid_p = centroid_projections[nearest_cluster];
      auto& centroid_s = centroid_sketches[nearest_cluster];

	  for (uint32_t l = 0; l < L; l++) {
        centroid_p[l] +=  static_cast<double>(graph_projections[gid][l]) /
                         (nearest_cluster_size+1);
        centroid_s[l] = centroid_p[l] >= 0 ? 1 : 0;
      }
	  cluster_sizes[nearest_cluster]++;

      // update anomaly score wrt. nearest cluster (centroid moved)
      anomaly_scores[gid] =  1.0-cos(PI*(1.0 - streamhash_similarity(graph_sketches[gid],centroid_s)));
    }
}
...
void update_frequency_distances_and_clusters(uint32_t gid,
  ...
  for (uint32_t i = 0; i < nclusters; i++) {
    distances[i] = 1-cosine_similarity(graph_edgeVectors[gid],centroid_edgeVectors[i]);
    if (distances[i] < min_distance) {
      min_distance = distances[i];
      nearest_cluster = i;
    }
  ...
  if (min_distance > cluster_thresholds[nearest_cluster]) 
  {
	...
	// set its anomaly score to distance from nearest centroid
      ...
      //update the cluster centroid
      int nearest_cluster_size = cluster_sizes[nearest_cluster];
      auto& centroid_v = centroid_edgeVectors[nearest_cluster];

      for (uint32_t l = 0; l < VECTOR_SIZE; l++) {
        centroid_v[l] += static_cast<double>(graph_edgeVectors[gid][l]) /
                         (nearest_cluster_size+1);
      }
	  cluster_sizes[nearest_cluster]++;

      // update anomaly score wrt. nearest cluster (centroid moved)
      anomaly_scores[gid] =  1-cosine_similarity(graph_edgeVectors[gid],centroid_v);
    }
  }
}
